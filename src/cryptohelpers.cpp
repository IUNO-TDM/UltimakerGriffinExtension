...
#else
string CryptoHelpers::CmErrorCodeAsString(){
    return "no wibu enabled build";
}
#endif

#ifndef NO_WIBU
void CryptoHelpers::CmDecrypt(unsigned long firm_code, unsigned long product_id, CryptoBuffer& buffer)
{
    CMULONG ulFirmCode = firm_code; //6000274;
...
#else
void CryptoHelpers::CmDecrypt(unsigned long, unsigned long, CryptoBuffer&)
{
    throw invalid_argument("Program requires wibu based decryption");
}
#endif
...
void CryptoHelpers::AesDecrypt(const CryptoBuffer& aes_key, CryptoBuffer& iv, CryptoBuffer& buffer, bool finalize){
    AES_KEY dec_key;
    AES_set_decrypt_key(aes_key, aes_key.size()*8, &dec_key);
    CryptoBuffer out(buffer.size());
    AES_cbc_encrypt(buffer, out, buffer.size(), &dec_key, iv, AES_DECRYPT);
    if(finalize){
